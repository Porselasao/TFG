function Emulti=fieldsum_jf(array_config,freq,R,th,phi)
...
%
% Components of Emulti (Etot,Evp...etc) are of the 
...
%
% ArrayCalc is intended for use as a far-field calculator, therefore the distance
% to the summing point (range_config) is ideally set to a value >>(2D^2/lambda).
%
% The default value for range_config is 999(m) set in init.m
%
% See also : fieldsum_old (version without 1/r loss).
...
for n=1:N                          % Sum over N sources
  ...
  rloc=sqrt(xloc.^2+yloc.^2+zloc.^2);
  thloc=acos(zloc./rloc);
  philoc=atan2(yloc,xloc);
 ...
 % [EleAmp,CPflag]=sumcode(eltype,thloc,philoc);   % Sumcode calls the relevant element model 'eltype'
  [EleAmp,CPflag]=Antenas_todas(thloc,philoc,freq,eltype);
 ...
 if CPflag==0                  % If linearly polarised
  ...
  Evpn=(1/rloc).*Amp.*EleAmp.*AmpVP.*exp(-j.*PhaVPt); % Vertical E-field component propagated to far-field point
  Ehpn=(1/rloc).*Amp.*EleAmp.*AmpHP.*exp(-j.*PhaHPt); % Horiz E-field component propagated to far-field point
...
EvpCplx=EvpSum*rloc/N;       % Scale Evp according to number of elements N and rloc
EhpCplx=EhpSum*rloc/N;       % Scale Ehp according to number of elements N and rloc
Evp=abs(EvpCplx);
Ehp=abs(EhpCplx);
...
Emulti=[Etot,Evp,Ehp,Elh,Erh,AR,Tau];              % Function output 
